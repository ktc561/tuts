...
struct SignInWithEmailView: View {
    //storing user info and auth state
    @EnvironmentObject var userInfo: UserInfo
    //user vm to handle verifications
    @State var user: UserViewModel = UserViewModel()
    //2 bound vars to receive from loginview so selected model can be presented on top of login viewv
