...
struct SignUpView: View {
    //env obj to store info and auth state
    @EnvironmentObject var userInfo: UserInfo
    //vm to do verification
    @State var user: UserViewModel = UserViewModel()
    //presentation mode to dismiss the sheet model 
            ...
            VStack {
                ...
                VStack(spacing: 20 ) {
                    Button(action: {
                        ...
                        FBAuth.createUser(withEmail: self.user.email, name: self.user.fullname, password: self.user.password) { (restult) in
                            switch restult {
